      ...
      switch (tipe) {
        case "person":
          ...
          for (Value x : vls) {
            ...
            if (!parse.filter((Resource) x, RDF.TYPE, ValueFactoryImpl.getInstance().createURI("http://vivoweb.org/ontology/core#Grant")).objects().isEmpty()) {
              download("Grant", base, str, (Resource) x, null, endp, true);
            }
        ...
        case "Grant":
          str.addAll(extractData(parse, "PREFIX foaf: <http://xmlns.com/foaf/0.1/> "
                  ...
                  + "construct { "
                  + " ?l a <http://xmlns.com/foaf/0.1/Project> . "
                  + " ?l <https://www.openaire.eu/cerif-profile/1.1/linksToPerson> ?pp . "
                  + " ?pp <https://www.openaire.eu/cerif-profile/1.1/MemberOf> ?l ."
                  + " ?l <http://purl.org/dc/terms/title> ?la ."
                  + "} where { values ?pp {<" + p.stringValue() + ">} . "
                  + "?pp rdfs:label ?la ."
        ...
        case "vcard":
          ...
          for (Value x : vlsx) {
            download("vcard_phone", base, str, (Resource) x, parent, endp, true);
          }
          break;
        case "vcard_phone":
          str.addAll(extractData(parse, "PREFIX foaf: <http://xmlns.com/foaf/0.1/> "
