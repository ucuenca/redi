...
public abstract class WorkerServiceImpl<S extends WorkerRuntime<T>, T extends WorkerConfiguration> implements WorkerService<S, T> {
  ...
  /**
   * The filters used by the enhancer to determine which resources are relevant.
   * The filters are and-connected, i.e. all filters need to match.
   */
  ...
  /**
   * This method is executed after rescheduling of resources in a configuration
   * takes place. Can be used to tun necessary cleanups after execution. By
   * default, does nothing.
   *
   * @param config
   */
   ...
   * replacing all previous enhancements. This method will clear the enhancement
   * graphs for all engines and place all resources into the enhancement queues
   * of all enhancement engines.
   */
  @Override
   ...
   * simply remove all enhancements for this engine and place all resources into
   * the enhancement queue for the given engine.
   *
  ...
  /**
   * Register the worker passed as argument with the service. Needs to be called
   * e.g. by the initialisation process
   *
   * @param runtime
   */
  @Override
  ...
  /**
   * Shutdown the enhancement service, abort all workers, terminate all threads,
   * and clean up any active resources.
   */
  @Override
  @PreDestroy
   ...
   * <p/>
   * This method should take care to ignore updates that have been triggered by
   * the enhancement process itself, or otherwise it might run into an
   ...
   * been added to the system. This method will trigger the creation of a queue
   * for the engine and trigger a complete rescheduling of all resources for the
   * given engine.
   *
   * @param engine
   */
