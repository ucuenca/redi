...
public class NameUtils {
  ...
  /*
    If a name (FirstName+LastName) has the pattern :  LastName2, FirstName MiddleName LastName1
    Consider as a simple fullName
    It is a common error in the Scopus Provider that causes duplicated profiles in REDI.
   */
  public static List<String> changeRealName(List<String> name1) {
    List<String> r = new ArrayList<>(name1);
    if (name1.size() == 2) {
      ModifiedJaccardMod utl = new ModifiedJaccardMod();
      String ffn = name1.get(0);
      String lln = name1.get(1);
      double fn = utl.countTokens(ffn);
      double ln = utl.countTokens(lln);
      if (fn > 2 * ln) {
        r.clear();
        r.add(ffn + " " + lln);
      }
  ...
  public static double compareName(List<String> name1, List<String> name2, boolean priorFirst) {
    ...
    switch (tipo) {
      ...
      case 22:
        if (tipo == 21) {
          metric.prioritizeWordOrder = false;
          Map.Entry<Integer, Double> simo = metric.distanceName(nc1, nf2 + " " + nl2);
          sim = simo.getValue();
          hasCompleteMatchs = simo.getKey() != 0;

          metric.priorOrd = true;
          Map.Entry<Integer, Double> sim1 = metric.distanceName(nl2, nc1);
          metric.priorOrd = false;
          if (sim1.getValue() == 0) {
            sim *= 0.5;
          }
        }
        if (tipo == 22) {
          ...
          metric.priorOrd = true;
          Map.Entry<Integer, Double> sim1 = metric.distanceName(nl1, nc2);
          metric.priorOrd = false;
          if (sim1.getValue() == 0) {
            sim *= 0.5;
          }
  ...
  public static List<String> bestName(List<List<String>> options) {
    ...
    for (int i = 0; i < options.size(); i++) {
      ...
      double v2 = 0;
      if (get.size() == 2) {
        double v21 = utl.countUniqueTokens(get.get(0) + " " + get.get(1));
        double v22 = utl.countUniqueTokens(get.get(1)) * 0.01;
        v2 = v21 + v22;
      } else if (get.size() == 1) {
        v2 = utl.countUniqueTokens(get.get(0));
      } else {
        v2 = 0;
      }
      double score = v1 + v2;
